...
function onCastSpell(creature, var)
	...
	if dmg > 0 then
	doAreaCombatHealth(creature, COMBAT_PHYSICALDAMAGE, position, area, math.min(0, min * dmg / (edmg + dmg)), math.min(0, max * dmg / (edmg+dmg)), CONST_ME_GROUNDSHAKER, ORIGIN_SPELL, true, crit)
	end
	if edmg > 0 then
	doAreaCombatHealth(creature, ItemType(weapon.itemid):getElementType(), position, area, math.min(0, min * edmg * dmgmultiplier / (edmg + dmg)), math.min(0, max * edmg * dmgmultiplier / (edmg+dmg)), (dmg > 0 and 0 or CONST_ME_GROUNDSHAKER), ORIGIN_SPELL, false, (dmg > 0 and 1 or crit))
	end
	return true
end
